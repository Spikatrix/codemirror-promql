...
function buildVectorMatching(state, binaryNode) {
    var e_1, _a, e_2, _b;
    if (!binaryNode || binaryNode.type.id !== parser_terms_1.BinaryExpr) {
        return null;
    }
    var result = {
    ...
    var binModifiers = binaryNode.getChild(parser_terms_1.BinModifiers);
    if (binModifiers) {
        var onOrIgnoring = binModifiers.getChild(parser_terms_1.OnOrIgnoring);
        if (onOrIgnoring) {
            result.on = onOrIgnoring.getChild(parser_terms_1.On) !== null;
            var labels = (0, path_finder_1.retrieveAllRecursiveNodes)(onOrIgnoring.getChild(parser_terms_1.GroupingLabels), parser_terms_1.GroupingLabelList, parser_terms_1.GroupingLabel);
            if (labels.length > 0) {
                try {
        ...
        var groupLeft = binModifiers.getChild(parser_terms_1.GroupLeft);
        var groupRight = binModifiers.getChild(parser_terms_1.GroupRight);
        if (groupLeft || groupRight) {
            ...
            var includeLabels = (0, path_finder_1.retrieveAllRecursiveNodes)(binModifiers.getChild(parser_terms_1.GroupingLabels), parser_terms_1.GroupingLabelList, parser_terms_1.GroupingLabel);
    ...
    var isSetOperator = (0, path_finder_1.containsAtLeastOneChild)(binaryNode, parser_terms_1.And, parser_terms_1.Or, parser_terms_1.Unless);
